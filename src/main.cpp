...
static const int64_t nStakeTargetSpacing = 1 * 60; // 1-minute stakes spacing
unsigned int nTargetSpacing = 30;               // block time 30 seconds
static const int64_t nTargetTimespan = 2 * 30;  // retarget every 1 min 
...
const string strMessageMagic = "Geld Signed Message:\n";
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64_t nBlockTargetSpacing = fProofOfStake ? nStakeTargetSpacing : nTargetSpacing; // 20 min for PoS / 30 sec PoW
    if(fProofOfStake) {
      //int64_t nTargetTimespan = (nStakeTargetSpacing * nInterval) ; // 20 min for PoS / 30 sec PoW
    }

    if (pindexLast == NULL)
        return bnTargetLimit.GetCompact(); // genesis block

    //int64_t nBlockDelaySpacing = nTargetSpacing + nTargetTimespan;
    ...
    printf(" nTargetSpacing = %d before bounds\n", nBlockTargetSpacing);
    if(fProofOfStake){ printf(" PoS / ProofOfStake: YES\n"); }
    CBigNum bnNew;
    ...
    if((pindexLast->nHeight <= LAST_POW_BLOCK && pindexLast->nHeight > 132714) || pindexLast->nHeight <= LAST_POW_BLOCK_FIRST)
    {
        if(pindexLast->nHeight < 4){
          ...
          if (nActualTimespan < (nBlockTargetSpacing - (nBlockTargetSpacing/8)) ) nActualTimespan = (nBlockTargetSpacing - (nBlockTargetSpacing/8));
          if (nActualTimespan > (nBlockTargetSpacing + (nBlockTargetSpacing/8)) ) {nActualTimespan = (nBlockTargetSpacing + (nBlockTargetSpacing/8));}
          else if (nActualTimespan > (nBlockTargetSpacing + (nBlockTargetSpacing/4)) ) {nActualTimespan = (nBlockTargetSpacing + (nBlockTargetSpacing/4));}
          else if (nActualTimespan > (nBlockTargetSpacing + (nBlockTargetSpacing/3)) ) {nActualTimespan = (nBlockTargetSpacing + (nBlockTargetSpacing/3));}
          else if (nActualTimespan > (nBlockTargetSpacing + (nBlockTargetSpacing/2)) ) {nActualTimespan = (nBlockTargetSpacing + (nBlockTargetSpacing/2));}
          else if (nActualTimespan > (nBlockTargetSpacing * nInterval) ) {nActualTimespan = (nBlockTargetSpacing * nInterval);}
        }
        if (nActualTimespan < 0) { nActualTimespan = ((nBlockTargetSpacing  - nInterval) );}
        printf(" nActualTimespan = %d after calculation\n", nActualTimespan);
        // Retarget
        bnNew *= nActualTimespan;
        bnNew /= nBlockTargetSpacing;
    }
    else {
      if (nActualTimespan <= 0) { nActualTimespan = nBlockTargetSpacing; }
      bnNew *= ((nInterval - 1) * nBlockTargetSpacing + (nActualTimespan * 2));
      bnNew /= ((nInterval + 1) * nBlockTargetSpacing);
    }
...
// geld: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Geld", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        CBigNum bnTarget; bnTarget.SetCompact(block.nBits);
